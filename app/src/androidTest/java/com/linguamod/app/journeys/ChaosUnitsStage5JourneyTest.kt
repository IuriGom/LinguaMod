...
/**
 * Stage 5 AC4: chaos-mode pass on units 11, 18 and 25.
 * Same fast-forward harness as [CompleteUnitsStage5JourneyTest], but the bot
 * answers wrong on purpose: up to 3 distinct exercises per lesson are failed
 * once (proving wrong answers re-queue and are then answered correctly), and
 * the checkpoint is first attempted with [SolverBot.chaosStaysWrong] so it
 * fails gracefully (retry screen, no crash, no progress recorded), then a
...
class ChaosUnitsStage5JourneyTest {
    ...
    @Test fun chaos_unit_11() = runChaosUnitJourney(11)
    @Test fun chaos_unit_18() = runChaosUnitJourney(18)
    @Test fun chaos_unit_25() = runChaosUnitJourney(25)
