...
/**
 * Stage 7 AC5/AC9: Story 4 "A cena dalla famiglia" (unlockAfterUnit 45)
 ...
 * Mirrors [Story3JourneyTest].
 */
@OptIn(ExperimentalTestApi::class)
@UninstallModules(AppModule::class)
@HiltAndroidTest
@RunWith(AndroidJUnit4::class)
class Story4JourneyTest {
     ...
     *  scroll-through itself can miss rows on the long 60-unit path under load). */
     ...
     * 60-unit path under load — swipe back up until the list can't scroll
    ...
    @Test
    fun journey_story4_correct_path_wrong_loops_and_rewards() {
        val plugin = loadPlugin()
        val story = plugin.stories.first { it.id == "story4" }
        seedCompletedUnits(45)
        runBlocking { featureUnlocks.unlock(FeatureUnlocks.STORY4) }

        composeRule.waitUntilExactlyOneExists(hasTestTag("path_list"), 30_000)
        openPathRow("story_row_story4", "story_screen")
        ...
        runBlocking {
            ...
            assertTrue(db.badgeDao().get(Badges.narratoreId("story4")) != null)
        }
